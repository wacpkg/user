...
CREATE OR REPLACE FUNCTION auth_mail.mail_set (mail_id u64, uid u64)
  RETURNS void
  LANGUAGE plpgsql
  AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION auth_mail.signup (client_id u64, mail_id u64, ctime u64, password_hash bytea)
  RETURNS u64
  LANGUAGE plpgsql
  AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION auth_mail.uid_by_mail_id (mail_id u64)
  RETURNS TABLE (
    user_id u64,
    hash bytea,
    ctime u64)
...
CREATE SEQUENCE IF NOT EXISTS auth_mail.user_id_seq START WITH 1
INCREMENT BY 1
NO MINVALUE
NO MAXVALUE
CACHE 1;
...
CREATE TABLE IF NOT EXISTS auth_mail."user" (
  id u64 DEFAULT nextval('auth_mail.user_id_seq'::regclass) NOT NULL, val u64 NOT NULL
);
